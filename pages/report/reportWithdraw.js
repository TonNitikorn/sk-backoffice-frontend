  ...
  const getReport = async (type, start, end) => {
    ...
    try {
      let res = await axios({
        ...
        method: "get",
        url: `${hostname}/api/report/withdraw/?start_date=${
      ...
      resData.map((item) => {
        item.no = no++;
        // item.name = item.member_account_banks[0].bank_account_name;
        // item.bank_name = item.member_account_banks[0].bank_name;
        // item.bank_number = item.member_account_banks[0].bank_number;
        // item.createdAt = moment(item.member_account_banks[0].createdAt).format(
        //   "DD/MM/YYYY hh:mm"
        // );
      });
      setReport(resData);
