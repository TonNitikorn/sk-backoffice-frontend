...
function memberTable() {
    ...
    try {
      let res = await axios({
        ...
        data: {
          ...
          affiliate_by:
            rowData.platform === "friend" ? rowData.affiliate_by : "-",
  ...
  const submitFormCredit = async (type) => {
    try {
      let time =
        moment(rowData.date).format("DD/MM") + "@" + hour + ":" + minute;
      ...
      if (!rowData.annotationWithdraw) {
        ...
        let res = await axios({
          headers: {
            ...
            menu: {
              page: "member_table",
              sub_menu: ["manage_deposit", "manage_withdraw"],
            },
          },
          method: "post",
  ...
  const columnsTran = [
    ...
    },
    {
      ...
      onFilter: (value, record) =>
        record.status_transction.indexOf(value) === 0,
    },
    {
      dataIndex: "create_at",
  ...
  return (
    <Layout>
      ...
      <Dialog
        ...
        onClose={() => {
          setOpenDialogEdit(false), setRowData({});
        }}
        fullWidth
        maxWidth="md"
      >
        <DialogTitle sx={{ mt: 2 }}>
          {" "}
          <EditIcon color="primary" /> แก้ไขข้อมูล : {rowData?.username}
        </DialogTitle>
            ...
            <Grid item xs={6}>
              ...
              <Chip
                ...
                sx={{
                  ...
                  fontSize: "14px",
                  backgroundColor:
                    openDialogManual.type === "deposit" ? "#16539B" : "#af2f2f",
                  color: "#ffffff",
                }}
              />
              ...
              {openDialogManual.type === "deposit" ? (
                <Grid container spacing={1}>
                  <Grid item xs={12}>
                    <Typography sx={{ fontSize: "14px", mb: 1, mt: 1 }}>
                      เวลาตามสลิป *
                    </Typography>
                  </Grid>
                  <Grid item xs={6}>
                  ...
                  <Grid item xs={3}>
                    <Autocomplete
                      ...
                      sx={{ width: 100 }}
                      renderInput={(params) => (
                        <TextField {...params} label="minute" size="small" />
                      )}
                    />
                  </Grid>
                </Grid>
              ...
              <Grid container item xs={12}>
                <Typography sx={{ fontSize: "14px", my: 1, mt: 2 }}>
                  หมายเหตุ *
                </Typography>
                <TextField
                ...
                >
                  ...
                  <MenuItem
                    value="รายการไม่เข้าธนาคาร"
                    onChange={() => setComment(0)}
                  >
                    รายการไม่เข้าธนาคาร
                  </MenuItem>
                  <MenuItem
                    value="เครดิตเข้าธนาคารไม่เข้าผู้ใช้"
                    onChange={() => setComment(0)}
                  >
                    เครดิตเข้าธนาคาร ไม่เข้าผู้ใช้
                  </MenuItem>
                  <MenuItem value="อื่นๆ" onChange={() => setComment(1)}>
                    อื่นๆ
                  </MenuItem>
                </TextField>
              </Grid>
      ...
      >
        <DialogTitle sx={{ mt: 1 }}>
          ...
          ประวัติการทำรายการของ : {rowData?.username}
        </DialogTitle>
                  ...
                  <>
                    <Table.Summary.Row>
                      ...
                      <Table.Summary.Cell>
                        ...
                        >
                          {Intl.NumberFormat("TH").format(parseInt(totalAfter))}
                        </Typography>{" "}
                      </Table.Summary.Cell>
                    </Table.Summary.Row>
                    <Table.Summary.Row>
                      ...
                      <Table.Summary.Cell>
                        ...
                        <Typography
                          ...
                          sx={{ fontWeight: "bold", color: "red" }}
                        >
                          {!totalSumCreditBefore
                            ? 0
                            : Intl.NumberFormat("TH").format(
                                parseInt(totalSumCreditBefore)
                              )}
                        </Typography>{" "}
                      </Table.Summary.Cell>
                      <Table.Summary.Cell>
