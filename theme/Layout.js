...
function Layout({ children, page }) {
  ...
  const getPermission = async () => {
    try {
      ...
      let tempReportFilter = []
      if (reportActive.length !== 0) {
        tempReportFilter = menuSuperAdmin.filter(item => reportActive.some((value) => value.menu.includes('report') === item.id.includes('report')));
      ...
      let tempBankFilter = []
      if (bankActive.length !== 0) {
        tempBankFilter = menuSuperAdmin.filter(item => bankActive.some((value) => value.menu.includes('bank') === item.id.includes('bank')));
        let banksub = tempBankFilter[0].bank.filter(item => bankActive.some((value) => value.menu === item.id));
        tempBankFilter[0].bank = banksub
      }

      let tempMemberFilter = []
      if (memberActive.length !== 0) {
        tempMemberFilter = menuSuperAdmin.filter(item => memberActive.some((value) => value.menu.includes('member') === item.id.includes('member')));
        for (const temp of tempMemberFilter) {
          if (temp.id === "member") {
            tempMemberFilter = [temp]
          }

        }

        let membersub = tempMemberFilter[0].member.filter(item => memberActive.some((value) => value.menu === item.id));
        tempMemberFilter[0].member = membersub

      }

      setMenuFilter([...tempmenFilter, ...tempBankFilter, ...tempMemberFilter, ...tempReportFilter,])
